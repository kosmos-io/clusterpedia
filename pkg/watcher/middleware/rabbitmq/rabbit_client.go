...
type RabbitClient struct {
	...
	codec          runtime.Codec
	started        bool
	cliStopCh      chan bool // stop client
	globalStopCh   <-chan struct{}
	expiresPerSend int
	notifyConfirm  chan amqp.Confirmation // msg send confirmed channel
	notifyClose    chan *amqp.Error       // channel closed event channel
	role           string
	newFunc        func() runtime.Object // event decode func
	queueExpires   int64
}
...
func (r *RabbitClient) initQuExchange() error {
	...
	// listen channel close event
	r.notifyClose = r.channel.NotifyClose(make(chan *amqp.Error, 1))
