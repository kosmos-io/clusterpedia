...
type RabbitClient struct {
	...
	codec          runtime.Codec
	started        bool
	cliStopCh      chan bool       // 停止单个client事件通道
	globalStopCh   <-chan struct{} // 全局停止事件通道
	expiresPerSend int
	notifyConfirm  chan amqp.Confirmation // 消息确认事件获取通道
	notifyClose    chan *amqp.Error       // channel异常事件获取通道
	role           string
...
func (r *RabbitClient) initQuExchange() error {
	...
	r.notifyClose = r.channel.NotifyClose(make(chan *amqp.Error, 1)) // 监听channel异常关闭信息

	if r.role == RoleProducer {
		err = r.channel.Confirm(false) // 设置消息确认
		if err != nil {
